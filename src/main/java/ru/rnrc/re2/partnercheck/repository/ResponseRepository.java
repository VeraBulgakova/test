...
public interface ResponseRepository extends JpaRepository<Response, Long> {
    @Modifying
    @Query(value = """
            WITH viewPhysical AS (
                    SELECT p.id,
                           pzinskey as partner_id
                    FROM index_olr_partnerlinkedstructure iop
                        ...
                        p.inn = iop.inn
                            ...
                            AND p.datelist = ?2
                    union
                    SELECT p.id, pzinskey as partner_id
                    FROM index_olr_partnerlinkedstructure iop
                             JOIN physical_person p ON
    ...
    @Query(value = """
            WITH viewLegal AS (
                ...
                insert into response(partner_id, check_object, linked_structure_order, participant_order
                                    , short_name, check_result, list_id)
                select  CAST(r.id AS text)    as partner_id,
                ...
                ;""", nativeQuery = true)
    void insertResponseRecordsFromTableForLegalPerson(String listName, String dateList);
